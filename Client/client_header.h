        ...
        try {
            ...
            switch (message->getType()) {
                ...
                case AUTH_PEER_KEY_EX:
                    ...
                    result = CryptoManager::rsa_decrypt(message->getPayload()->getCiphertext(), message->getCTxtLen(),
                                                        &plaintext,
                ...
                case ERROR:
                    ...
                    switch (message->getErrCode()) {
                        case FORWARD_ACCEPT_FAIL:
                            cerr << "Server unable to accept the request to talk. The request has been nullified"
                                 << endl;
                            m_status.lock();
                            is_busy = false;
                            m_status.unlock();
...
int verify_server_authenticity(Message *message, bool peer_protection) {
    ...
    result = CryptoManager::verify_auth_data(aad, aad_len, message->getIv(), sever_session_key,
                                             peer_protection ? message->getServerAuthTag()
